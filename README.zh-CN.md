...
- **认证清单已由维护者对全部五个现行模块版本离线签名**（`us-msb@2026.07.1`、
  `uk/eu/sg-msb@2026.08.1`、`ae-msb@2026.08.2`，签名地址 `0x1423BD…8e92`）。
  买到不在清单里的模块版本，案件会在验证器检查②以 `attestation_missing` 失败、
  走 reject 路径、escrow 退回客户——fail-closed，且在 2026-08-04 规则版本轮换
  期间有过真实链上实证。
...
验证器是独立的**角色**、独立的**密钥**——SA 绝不自证——但**不是独立部署的进程**。
当前线上以 `VERIFIER_MODE=in-process` 与主服务同进程运行，一个服务同时持有运营与
验证器两把钥；远程验证器模式代码已具备，拆分部署是计划项、尚未上线。）
